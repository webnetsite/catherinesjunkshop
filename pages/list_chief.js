...
const ListChief = ({chiefs}) => {
 ...
 const options = {
    onRowsDelete: async (e) => {
      ...
      const endpoint = `/api/chiefs/${chiefs[index].id}`
      const options = {
        method: 'DELETE',
        headers: {
 ...
 const columns = [
  ...
  { name: "ao", label: "Approving Officer",
     options: {
       customBodyRenderLite: (dataIndex, rowIndex) => {
         let ok = chiefs[dataIndex].ao === 1 ? "yes" : "no";
         return (<div>{ok} </div> )
       }
     }  
  },
  { name: "ro", label: "Recommending Officer",
     options: {
       customBodyRenderLite: (dataIndex, rowIndex) => {
         let ok = chiefs[dataIndex].ro === 1 ? "yes" : "no";
         return (<div>{ok} </div> )
       }
     } 
  },
  { name: "ta", label: "TO Approval",
     options: {
       customBodyRenderLite: (dataIndex, rowIndex) => {
         let ok = chiefs[dataIndex].ta === 1 ? "yes" : "no";
         return (<div>{ok} </div> )
       }
     } 
  },
  { name: "fp", label: "Fuel Prepared",
     options: {
       customBodyRenderLite: (dataIndex, rowIndex) => {
         let ok = chiefs[dataIndex].fp === 1 ? "yes" : "no";
         return (<div>{ok} </div> )
       }
     } 
  },
  { name: "fa", label: "Fuel Approved",
     options: {
       customBodyRenderLite: (dataIndex, rowIndex) => {
         let ok = chiefs[dataIndex].fa === 1 ? "yes" : "no";
         return (<div>{ok} </div> )
       }
     } 
  },
       ...
       customBodyRenderLite: (dataIndex, rowIndex) => {
         return (
           <Link className="shadow bg-cyan-700 hover:bg-cyan-500 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded"
             href={`/chief/${chiefs[dataIndex].id}`} replace>
             Edit
           </Link>
         );
       }
