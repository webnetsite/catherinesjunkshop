...
const StationUpdate = ({ tt }) => {
  const metaInfo = {
    title: "Update Gasoline Station"
  };
  ...
  const handleSubmit = async (event) => {
    ...
    const endpoint = '/api/stations/update_station';
    const options = {
      method: 'POST',
      headers: {
    ...
    setModalContent(result.success ? "Data Updated Successfully" : "Update Failed");
    setIsModalOpen(true);
  };
          ...
          <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
              ...
              <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <button type="button" onClick={closeModal} className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-blue-600 text-base font-medium text-white hover:bg-blue-700 sm:ml-3 sm:w-auto sm:text-sm">
                  Close
                </button>
              </div>
            </div>
...
export async function getServerSideProps(context) {
  const id = Number(context.params.edit);

  const tt = await prisma.vendor.findUnique({
    where: { id }
  });
